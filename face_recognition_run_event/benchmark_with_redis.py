...
def main():
    ...
    output_dir = os.path.join(settings.BASE_DIR, 'benchmark_output_redis')
    ...
    print(f'Running REDIS benchmark with {len(query_vectors)} queries x {repeats} repeats...')
    ...
    for qidx, qvec in enumerate(query_vectors, start=1):
        ...
        for ridx in range(1, repeats + 1):
            ...
            cache_key = f'redis_bench_face_{qidx}_{THRESHOLD}'
    ...
    csv_path = os.path.join(output_dir, 'benchmark_with_redis.csv')
    ...
    try:
        ...
        plt.title('Benchmark - With Redis')
        for bar, val in zip(bars, avgs):
            plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{val:.4f}', ha='center', va='bottom')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'benchmark_with_redis_bar.png'), dpi=200)
        plt.close()

        plt.figure(figsize=(10, 5))
        plt.boxplot([no_cache_times, cache_miss_times, cache_hit_times], tick_labels=labels)
        plt.ylabel('Time (s)')
        plt.title('Benchmark - With Redis Distribution')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'benchmark_with_redis_boxplot.png'), dpi=200)
        plt.close()
